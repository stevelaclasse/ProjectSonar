...
public class Tasks {
...

	{
		...
		myUtils.writeUrlLangageJavaInFile(myUrlsUniqueEntry, http.getLangageJavaUrlsFileName());
		...
		myUtils.writeUrlMavenProjectInFile(myUrlsLangageJava, http.getMavenProjectUrlsFileName()); // sort only Maven
																									// Project

		myUtils.writeUrlGradleProjectInFile(myUrlsLangageJava, http.getGradleProjectUrlsFileName()); // sort only Gradle
																										// Project
		...
		gradleProjectUrl = myUtils.readUrlFromFile(http.getGradleProjectUrlsFileName()); // read all the Gradle Project
																							// Urls
	...
	protected void launchBatCommandInBatFiles() throws Exception {
		...
		utilsBatExecutor.executeCommandInBatFile(http.getJacocoPluginProjectMavenGitCommandLineBatFileName(),
				http.getGitGoodDownloadedProjectFileName());
		System.out.println("Begining with GIT JACOCO GRADLE");
		utilsBatExecutor.executeCommandInBatFile(http.getJacocoPluginProjectGradleGitCommandLineBatFileName(),
				http.getGitGoodDownloadedProjectFileName());
		System.out.println("Begining with GIT COBERTURA MAVEN");
		utilsBatExecutor.executeCommandInBatFile(http.getCoberturaPluginProjectMavenGitCommandLineBatFileName(),
				http.getGitGoodDownloadedProjectFileName());
		System.out.println("Begining with GIT COBERTURA GRADLE");
		utilsBatExecutor.executeCommandInBatFile(http.getCoberturaPluginProjectGradleGitCommandLineBatFileName(),
				http.getGitGoodDownloadedProjectFileName());
	...
	protected void extractingAnalyseResultsFromSonarQubeServerIntoExcelFile() throws Exception {
		...
		if (responseCode == 200) {
			...
			String[] sheetTitle = { "PROJECTNAME", "BRANCH_COVERAGE", "LINE_COVERAGE", "COVERAGE", "TESTS",
					"CODE_SMELLS", "SQALE_RATING", "SQALE_INDEX", "DUPLICATED_LINES", "DUPLICATED_LINES_DENSITY",
					"LINES", "BUGS", "RELIABILITY_RATING", "RELIABILITY_REMEDIATION_EFFORT", "COMPLEXITY",
					"VULNERABILITIES", "SECURITY_REMEDIATION_EFFORT" };

			String[] sheetRawTitle = { "PROJECTNAME", "BRANCH_COVERAGE", "LINE_COVERAGE", "COVERAGE", "TESTS",
					"CODE_SMELLS", "MAINTAINABILITY RATING", "TECHNICAL_DEBT", "DUPLICATED_LINES",
					"DUPLICATED_LINES_DENSITY (%)", "LINES", "BUGS", "RELIABILITY_RATING",
					"RELIABILITY_REMEDIATION_EFFORT", "COMPLEXITY", "VULNERABILITIES", "SECURITY_REMEDIATION_EFFORT" };

			for (int i = 0; i < myComponents.components.size(); i++) {

				http.setPath("api/measures/component?componentId=" + myComponents.components.get(i).id
						+ "&metricKeys=branch_coverage,"
				...
				if (responseCode == 200) {
					...
					for (int j = 1; j < sheetTitle.length; j++) {
						...
						System.out.println("Initialising -->" + sheetTitle[j] + ":" + data.get(sheetTitle[j]));
					}

					for (int j = 0; j < myMetrics.component.measures.size(); j++) {

						data.put(myMetrics.component.measures.get(j).metric.toUpperCase(),
			...
			for (int i = 0; i < allComponents.size(); i++) {
				...
				for (int k = 0; k < sheetTitle.length; k++) {
					...
					if (allComponents.get(i).get(sheetTitle[k]) != null) {
						...
						if (k > 0) {
							System.out.println("metric:" + sheetTitle[k] + "-->adding Number Value:"
									+ Float.parseFloat(allComponents.get(i).get(sheetTitle[k])) + " \t in cell:"
									+ rowNum + "->" + cellNum);
							myCell.setCellValue(Float.parseFloat(allComponents.get(i).get(sheetTitle[k])));
						} else {
							System.out.println("metric:" + sheetTitle[k] + "adding Text Value:"
									+ allComponents.get(i).get(sheetTitle[k]) + "\t in cell:" + rowNum + "->"
									+ cellNum);
